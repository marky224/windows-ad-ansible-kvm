...
| Phase 6 — Multi-site (2nd DC + AD sites) | 🚧 In progress | Design locked (ADR-052). **Done:** isolated Branch subnet (`corp-branch`, 10.20.0.0/24) + **VyOS inter-site router** (`VYOS01`, ~40 ms netem WAN); **AD Sites** (HQ/Branch) with **ADDC02** promoted as a writable Branch-Site replica DC + GC (bidirectional replication verified; self-first DNS); **reciprocal hot-standby DHCP failover** + cross-site relay; a read-only **two-site verification** gate (`verify-multisite.yml`); and an **isolated-sandbox FSMO-seize DR rehearsal** (`ops_firedrill` seize). **Pending:** a live, non-destructive failover drill |
