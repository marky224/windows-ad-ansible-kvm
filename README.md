...
| Phase 6 — Multi-site (2nd DC + AD sites) | 🚧 In progress | Design locked (ADR-052). **Done:** Branch subnet + **VyOS inter-site WAN** (~40 ms netem); **AD Sites** (HQ/Branch) with **ADDC02** as a writable Branch replica DC + GC (bidirectional replication, self-first DNS); reciprocal **hot-standby DHCP failover** + cross-site relay; read-only **two-site verification** (`verify-multisite.yml`); and **FSMO-seize + live failover DR drills**. **Pending:** cross-site hardening follow-ups |
