...
| Phase 6 — Multi-site (2nd DC + AD sites) | 🚧 In progress | Design locked (ADR-052). **Done:** Branch subnet + **VyOS inter-site WAN** (~40 ms netem); **AD Sites** (HQ/Branch) with **ADDC02** as a writable Branch replica DC + GC (bidirectional replication, self-first DNS); reciprocal **hot-standby DHCP failover** + cross-site relay; read-only **two-site verification** (`verify-multisite.yml`); and **FSMO-seize + live failover DR drills — both green** (the live drill found and fixed a real cross-site routing gap: HQ clients now get the branch route via **DHCP option 121**, so DNS/auth fail over to ADDC02 during an HQ outage). **Pending:** final branch-state snapshots |
